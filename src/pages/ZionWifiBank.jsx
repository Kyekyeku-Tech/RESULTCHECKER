    ...
    <div className="overflow-hidden relative w-full h-full flex justify-center items-center">
      <AnimatePresence initial={false} mode="wait">
        ...
        >
          ...
          <a
            href={`https://wa.me/233593225000?text=${encodeURIComponent(whatsappPrefix(items[index]))}`}
            target="_blank"
...
export default function ZionWifiBank() {
  ...
  const apiKey = import.meta.env?.VITE_MNOTIFY_KEY || process.env.REACT_APP_MNOTIFY_KEY || "zov6giqNXoQeyMdHQeqgXkOCa";
  const senderId = "KyekyekuTec";

  const toggleTheme = () => setTheme(theme === "dark" ? "light" : "dark");

  const installationItems = [
    { title: "Bulk Purchase", price: "GHS 19.0 = 10-19 Checkers", desc: "Get your wassce Result in pdf after payment." },
    { title: "Single purchance", price: "GHS 20.0 = 1-9 checkers", desc: "Get your wassce Result in pdf after payment." },
    { title: "XBulk Puchanse", price: "GHS 17.0 = 20+ checkers", desc: "For Printing press and more get discount." },
    { title: "Bulk Purchase", price: "GHS 19.0 = 10-19 Checkers", desc: "Get your wassce Result in pdf after payment." },
    { title: "Single purchance", price: "GHS 20.0 = 1-9 checkers", desc: "Get your wassce Result in pdf after payment." },
    { title: "XBulk Puchanse", price: "GHS 17.0 = 20+ checkers", desc: "For Printing press and more get discount." },
  ];
  ...
  const dedicatedItems = [
    { speed: "1-9 Checkers", price: "GHS 20 ", desc: "NB: All packages are Available 3 times check." },
    { speed: "10-19 Checkers", price: "GHS 19 ", desc: "NB: All packages are Available 3 times check." },
    { speed: "20 - More", price: "GHS 17 ", desc: "NB: All packages are Available 3 times check." },
   { speed: "1-9 Checkers", price: "GHS 20 ", desc: "NB: All packages are Available 3 times check." },
    { speed: "10-19 Checkers", price: "GHS 19 ", desc: "NB: All packages are Available 3 times check." },
    { speed: "20 - More", price: "GHS 17 ", desc: "NB: All packages are Available 3 times check." },
  ];
    ...
    async function loadPackages() {
      try {
        ...
        const pkgs = docs.length
          ? docs
          : [
              { id: "bronze", name: "Bronze", price: 19, description: "3 Times Availability without Pdf" },
              { id: "silver", name: "Silver", price: 20, description: "3 Times Availability With Pdf" },
              { id: "gold", name: "Gold", price: 20, description: "3 Times Availability With Pdf" },
  ...
  const generateTicket = ({ reference, pkg, username, password }) => {
    ...
    docPDF.text("Wassce Checker – Access Ticket", 20, 20);
    ...
    ctx.fillStyle = "white";
    ctx.font = "28px Arial";
    ctx.fillText("Kyekyeku Tech  Checker Ticket", 150, 50);
...
try {
  ...
  const handler = window.PaystackPop.setup({
    key: PAYSTACK_KEY,
    email: email || "no-reply@kyekyeku.tech",
  ...
  const handlePaymentSuccess = async (reference, pkg) => {
    ...
    try {
      ...
      try {
        const smsMessage = `Hello ${name}, your Wassce Checker is ready!
Package: ${pkg.name}
SERIAL: ${credData.username}
PIN: ${credData.password}
Reference: ${reference}
ghana.waecdirect.org
whatsapp: +233593225000
Thank you for choosing Kyekyeku-Tech.`;
    ...
    <div className={containerClass}>
      <div className="max-w-5xl mx-auto">
        <header className="mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
          <div>
            <h1 className={theme === "dark" ? "text-3xl font-bold text-sky-400 mb-2" : "text-3xl font-bold text-blue-600 mb-2"}>Auto WASSCE Result Checker</h1>
            <p className={theme === "dark" ? "text-slate-200" : "text-gray-700"}>Fast — Easy — Reliable</p>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <a href="tel:0545454000" className="bg-yellow-400 text-slate-900 px-4 py-2 rounded-full font-bold hover:bg-yellow-300">Call Admin</a>
        ...
        <section className="mt-6 relative h-[300px] md:h-[250px]">
          <h2 className="text-2xl font-bold text-blue-700 text-center mb-4">Bulk Purchase</h2>
          <AutoCarousel items={installationItems} theme={theme} whatsappPrefix={(item) => `Hello Admin, I want the ${item.title}.`} />
        </section>

        <section className="mt-6 relative h-[300px] md:h-[250px]">
          <h2 className="text-2xl font-bold text-blue-700 text-center mb-4">Bulk Packages</h2>
          <AutoCarousel items={dedicatedItems} theme={theme} whatsappPrefix={(item) => `Hello Admin, I want to order the ${item.speed} Bulk Checker Package.`} />
        </section>
      </div>
    </div>
  );
